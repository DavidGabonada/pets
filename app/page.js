...
export default function Dashboard() {
  ...
  const fetchPets = async () => {
    try {
      const url = 'http://localhost/pets/pets.php';
  ...
  useEffect(() => {
    const fetchSpecies = async () => {
      try {
        ...
        const response = await axios.post("http://localhost/pets/pets.php", formData);
  ...
  const handleAddBreed = async (e) => {
    ...
    try {
      ...
      const formData = new FormData();
      formData.append("json", JSON.stringify(jsonData));
      formData.append("operation", "addBreeds");
      const res = await axios.post(url, formData);
      console.log(res.data)
      if (res.data === 1) {
  ...
  return (
    <>
      <Container className="mt-5" style={{ background: 'linear-gradient(to bottom, #ff69b4  , #ffd700)', padding: '5px', minHeight: '100vh' }}>
        <div>
          <h1 className="text-center" style={{ color: 'white' }}>PETSHOP NI DABID</h1>
